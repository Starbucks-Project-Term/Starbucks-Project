...
app.post('/favdata', (request, response) => {
    ...
    loadUserdata(logged_in.username).then(res => {
        ...
        for (var i = 0; i < saved_loc.length; i++) {
         ...
         displaySaved += `<div id=s${saved_loc.length} class="favItems"><a onclick="getMap(${last_save})"> ${last_save}</a><button <button id="del${i}" class="delButton" onclick="deleteFav(${i})">x</button></div>`;
